...
OUT_PATH = "./data/distorted_plane.obj"
...
# Save parameters to a .txt file next to the OBJ
PARAM_PATH = OUT_PATH.replace(".obj", ".txt")

with open(PARAM_PATH, "w") as pf:
    pf.write("Distorted Glass Parameters\n")
    pf.write("===========================\n\n")
    pf.write(f"SIZE_X = {SIZE_X}\n")
    pf.write(f"SIZE_Z = {SIZE_Z}\n")
    pf.write(f"GRID   = {GRID}\n")
    pf.write(f"AMP    = {AMP}\n\n")

    pf.write("Sinusoidal Frequencies:\n")
    pf.write(f"  f1x = {f1x}, f1z = {f1z}\n")
    pf.write(f"  f2x = {f2x}, f2z = {f2z}\n\n")

    pf.write("Bumps (x0, z0, radius, amplitude):\n")
    for b in bumps:
        pf.write(f"  {b}\n")
    pf.write("\n")

    pf.write("Laplacian smoothing:\n")
    pf.write("  lam = 0.16\n")
    pf.write("  iters = 3\n")

print("Saved OBJ to:", OUT_PATH)
